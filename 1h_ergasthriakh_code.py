...
#erwthma 1a
fm  = 4000 # (0+5+8=13 => 1+3=4 => fm=4kHz
fs1 = fm * 25
fs2 = fm * 60
fs3 = fm * 5
A = 4  #πλάτος
...
sign = 1
bit_stream = [0]*(8*np.size(gray_function))
for i in range(np.size(gray_function)):
    for j in range(4):
        bit_stream[8*i+2*j] = (gray_function[i]//10**(4-j))%2*sign*4 #Η bit_stream έχει τα bits που προκύπτουν από την
          #gray_με κωδικοποίση γραμμής BIPOLAR RZ
        if ((gray_function[i]//10**(4-j))%2 == 1):
            sign = sign*(-1)
        bit_stream[8*i+2*j+1] = 0
